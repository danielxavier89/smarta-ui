...
4. **A row with `onActivate` may still carry its own controls.** A button, link or menu inside the row handles its own click and its own Enter, and the row does not fire a second time on top of it. This is the one place `Table` differs from `Card`, which cannot nest interactive elements at all.
